import { useState, useEffect } from 'react';
import init, { derive_key, encrypt, decrypt } from "my_crypto_lib";
import { Plus, Copy, Eye, EyeOff } from 'lucide-react';

export default function Vault() {
  const [secrets, setSecrets] = useState([]);
  const [masterKey, setMasterKey] = useState(null);
  
  // Form State
  const [isAdding, setIsAdding] = useState(false);
  const [service, setService] = useState('');
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [visibleID, setVisibleID] = useState(null);

  useEffect(() => {
    // We assume the Master Key was passed or stored in session (simplified for now)
    // For this demo, we re-ask for the key or derive it from session storage if you prefer.
    // Let's prompt user if key is missing.
    init();
  }, []);

  // Helper to re-derive key if page refreshed (Security tradeoff: ask user again usually)
  const unlockVault = (username, masterPass) => {
    const key = derive_key(masterPass, username);
    setMasterKey(key);
    fetchSecrets(key);
  };

  const fetchSecrets = async (key) => {
    const token = localStorage.getItem('token');
    const API_URL = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
    
    const res = await fetch(`${API_URL}/api/secrets/`, {
      headers: { 'Authorization': `Token ${token}` }
    });
    const data = await res.json();
    
    const decrypted = data.map(s => {
      try {
        // We decrypt the JSON string: "{email: '...', pass: '...'}"
        const rawJson = decrypt(key, s.ciphertext);
        return { ...s, ...JSON.parse(rawJson) };
      } catch (e) {
        return { ...s, email: '???', pass: '⚠️ Decryption Failed' };
      }
    });
    setSecrets(decrypted);
  };

  const handleSave = async (e) => {
    e.preventDefault();
    if (!masterKey) return alert("Vault locked!");

    // 1. Pack data into JSON
    const payload = JSON.stringify({ email, pass: password });
    
    // 2. Encrypt the JSON blob
    const blob = encrypt(masterKey, payload);

    // 3. Send
    const token = localStorage.getItem('token');
    const API_URL = import.meta.env.VITE_API_URL || 'http://127.0.0.1:8000';
    
    await fetch(`${API_URL}/api/secrets/`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Token ${token}` },
      body: JSON.stringify({ name: service, ciphertext: blob })
    });

    setIsAdding(false);
    fetchSecrets(masterKey);
    setService(''); setEmail(''); setPassword('');
  };

  // If locked, show mini unlock screen
  if (!masterKey) {
    return (
      <div className="auth-container">
        <h2>🔓 Unlock Vault Session</h2>
        <p>Please re-enter keys to decrypt your data.</p>
        <form onSubmit={(e) => {
          e.preventDefault();
          unlockVault(e.target.user.value, e.target.pass.value);
        }}>
          <div className="form-group">
            <input name="user" placeholder="Username" required />
          </div>
          <div className="form-group">
            <input name="pass" type="password" placeholder="Master Key" required />
          </div>
          <button className="btn-primary">Decrypt</button>
        </form>
      </div>
    );
  }

  return (
    <div className="container">
      <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
        <h2>My Passwords</h2>
        <button className="btn-primary" onClick={() => setIsAdding(!isAdding)} style={{width: 'auto'}}>
          <Plus size={18} style={{verticalAlign: 'middle'}}/> Add New
        </button>
      </div>

      {isAdding && (
        <div className="auth-container" style={{maxWidth: '600px'}}>
          <h3>Add New Password</h3>
          <form onSubmit={handleSave}>
            <div className="form-group"><input placeholder="Service (e.g. Netflix)" value={service} onChange={e=>setService(e.target.value)} required /></div>
            <div className="form-group"><input placeholder="Email / Username" value={email} onChange={e=>setEmail(e.target.value)} required /></div>
            <div className="form-group"><input type="password" placeholder="Password" value={password} onChange={e=>setPassword(e.target.value)} required /></div>
            <button className="btn-primary">Encrypt & Save</button>
          </form>
        </div>
      )}

      <div className="vault-grid">
        {secrets.map(s => (
          <div key={s.id} className="secret-card">
            <div className="secret-header">
              <h3>{s.name}</h3>
              <Shield size={20} color="#3b82f6"/>
            </div>
            <div style={{marginBottom: '10px', color: '#94a3b8'}}>{s.email}</div>
            
            <div style={{display: 'flex', gap: '10px', alignItems: 'center'}}>
              <div className="secret-value">
                {visibleID === s.id ? s.pass : '••••••••••••'}
              </div>
              <button onClick={() => setVisibleID(visibleID === s.id ? null : s.id)} style={{background: 'none', border: 'none', cursor: 'pointer', color: 'white'}}>
                {visibleID === s.id ? <EyeOff size={18}/> : <Eye size={18}/>}
              </button>
            </div>
          </div>
        ))}
      </div>
    </div>
  );
}